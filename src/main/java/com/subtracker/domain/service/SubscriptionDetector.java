...
/**
 * 거래 내역에서 구독 서비스를 자동으로 감지하는 서비스
 */
@Slf4j
public class SubscriptionDetector {
    
    // 구독 감지 설정값
    private static final int MIN_OCCURRENCE = 2;           // 최소 발생 횟수
    private static final double AMOUNT_TOLERANCE = 5.0;    // 금액 허용 오차 (%)
    private static final int MAX_DAY_VARIANCE = 5;         // 결제일 최대 오차 (일)
    
    /**
     * 거래 내역에서 구독 서비스 감지
     * 
     * @param transactions 전체 거래 내역
     * @return 감지된 구독 서비스 목록
     */
    public List<Subscription> detectSubscriptions(List<Transaction> transactions) {
        ...
        for (Map.Entry<String, List<Transaction>> entry : merchantGroups.entrySet()) {
            ...
            // 최소 발생 횟수 체크
            if (merchantTransactions.size() < MIN_OCCURRENCE) {
                continue;
            }
            ...
            if (subscription != null) {
                ...
                log.info("구독 감지: {} - {}원/월", 
                    subscription.getServiceName(), subscription.getMonthlyAmount());
            }
        }
    ...
    private boolean hasConsistentAmount(List<Transaction> transactions) {
        ...
        List<BigDecimal> amounts = transactions.stream()
            ...
            .filter(amount -> amount.compareTo(BigDecimal.ZERO) > 0)  // 양수만
            .sorted()
        ...
        long consistentCount = amounts.stream()
            .filter(amount -> isWithinTolerance(amount, median, AMOUNT_TOLERANCE))
            .count();
    ...
    private Subscription.BillingCycle detectBillingCycle(List<Transaction> transactions) {
        ...
        // 주기 판단 (오차 범위 고려)
        if (isInRange(avgInterval, 30, MAX_DAY_VARIANCE)) {
            return Subscription.BillingCycle.MONTHLY;
        } else if (isInRange(avgInterval, 90, MAX_DAY_VARIANCE * 2)) {
            return Subscription.BillingCycle.QUARTERLY;
        } else if (isInRange(avgInterval, 180, MAX_DAY_VARIANCE * 3)) {
            return Subscription.BillingCycle.SEMI_ANNUAL;
        } else if (isInRange(avgInterval, 365, MAX_DAY_VARIANCE * 5)) {
